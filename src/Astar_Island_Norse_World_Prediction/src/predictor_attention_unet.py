...
def _load_attn_unet(model_path: Path) -> AttentionUNet:
    global _ATTN_UNET_MODEL
    if _ATTN_UNET_MODEL is None:
        model = AttentionUNet()
        if model_path.is_file():
            model.load_state_dict(torch.load(model_path, map_location="cpu"))
        model.eval()
        _ATTN_UNET_MODEL = model
    return _ATTN_UNET_MODEL

def build_prediction_tensor_attn_unet(
    ...
    if model_path is None:
        model_path = Path("data/reports/attention_unet_predictor.pth")
    ...
    model = _load_attn_unet(model_path)

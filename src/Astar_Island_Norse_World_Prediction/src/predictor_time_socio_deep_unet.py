...
class TimeSocioDeepUNet(nn.Module):
    def __init__(self, in_channels=21, out_channels=6):
        ...
        self.enc4 = ResidualConv(384, 768)
        ...
        self.att1 = AttentionGate(F_g=384, F_l=384, F_int=192)
        self.dec1 = ResidualConv(768, 384)
        
        self.up2 = nn.ConvTranspose2d(384, 192, kernel_size=2, stride=2)
        self.att2 = AttentionGate(F_g=192, F_l=192, F_int=96)
        self.dec2 = ResidualConv(384, 192)
        
        self.up3 = nn.ConvTranspose2d(192, 96, kernel_size=2, stride=2)
        self.att3 = AttentionGate(F_g=96, F_l=96, F_int=48)
        self.dec3 = ResidualConv(192, 96)
    ...
    def forward(self, x):
        ...
        t_feat = self.temp_pool(e4).view(e4.size(0), -1)
...
_TIME_SOCIO_DEEP_UNET_MODEL = None

def _load_time_socio_deep_unet(model_path: Path) -> TimeSocioDeepUNet:
    global _TIME_SOCIO_DEEP_UNET_MODEL
    if _TIME_SOCIO_DEEP_UNET_MODEL is None:
        model = TimeSocioDeepUNet(in_channels=21, out_channels=6)
        if model_path.is_file():
            model.load_state_dict(torch.load(model_path, map_location="cpu"))
        model.eval()
        _TIME_SOCIO_DEEP_UNET_MODEL = model
    return _TIME_SOCIO_DEEP_UNET_MODEL

def build_prediction_tensor_time_socio_deep_unet(
    ...
    if model_path is None:
        model_path = Path("data/reports/time_socio_deep_unet_predictor.pth")
    ...
    model = _load_time_socio_deep_unet(model_path)
    ...
    with torch.no_grad():
        ...
        for k, flip in rot_flip_pairs:
            ...
            x_final_aug = np.concatenate(
                [x_aug, dist_aug[np.newaxis, :, :], time_channel_aug, obs_channels_aug, socio_channels_aug, obs_mask_aug],
                axis=0,
            )
